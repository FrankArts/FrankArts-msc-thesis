...
# NOTE: reach task is not used in final version of the thesis.

# Possible values and synonyms for each feature:
COLORS = {"red": ["red", "crimson", "ruby", "scarlet"], "blue": ["blue", "aqua", "teal", "azure"]}
...
def generate_pickup_instructions(color, shape):
    #Generate four different instructions for a pickup task with the given color/shape
    output = []
...
def generate_reach_instructions(color, shape):
    #Generate four different instructions for a reach task with the given color/shape
    output = []
...
#Generate instructions for all possible colors (values and synonyms) and shapes (values and synonyms)
all_tasks = []
...
#with open(FILE_PATH + "/task_instructions.pickle", 'wb') as handle:
...
#uncomment to save task ids
#with open(FILE_PATH + "/task_ids.pickle", 'wb') as handle:
